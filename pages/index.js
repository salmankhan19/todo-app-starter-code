...
export default function Home() {
    ...
    const addTodo = async () => {
        try {
            const decRef = await addDoc(collection(db , "todos"),{
                owner: authUser.uid,
                content: todoInput,
                completed: false,
            });
    ...
    return !authUser ? <Loader/> : (
        <main className="">
            <div className="bg-black text-white w-44 py-4 mt-10 rounded-lg transition-transform hover:bg-black/[0.8] active:scale-90 
            flex items-center justify-center gap-2 font-medium shadow-md fixed bottom-5 right-5 
                ...
                <div className="bg-white -m-6 p-3 sticky top-0">
                    ...
                    <div className="flex items-center gap-2 mt-10">
                        <input
                            ...
                            onKeyUp={onKeyUp}
                        />
                        <button className="w-[60px] h-[60px] rounded-md bg-black flex justify-center items-center 
                        cursor-pointer transition-all duration-300 hover:bg-black/[0.8]" onClick={addTodo}>
